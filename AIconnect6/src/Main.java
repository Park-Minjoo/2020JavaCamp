...
public class Main extends JFrame {
	...
	static int count = 16;
	static Timer t;
	static int[][] weightGraph = new int[19][19];
...
//					lateFrame.setBackground(new Color(119, 136, 153));
		...
		ai.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				turnB = true;
				// 중앙값 350, 400 한칸 25
				int x= ((int)(Math.random() * 2) + 9) * 25 + 80;
				int y= ((int)(Math.random() * 2) + 9) * 25 + 130;
				
				for(int i = 0; i < Ai.redDol.size(); i++) {
					if(Ai.redDol.get(i).x == x && Ai.redDol.get(i).y == y) {
						while(true) {
							x = ((int)(Math.random() * 2) + 9) * 25 + 80;
							y = ((int)(Math.random() * 2) + 9) * 25 + 13;
